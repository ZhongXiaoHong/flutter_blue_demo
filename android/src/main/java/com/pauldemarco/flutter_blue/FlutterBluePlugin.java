    ...
    public void onMethodCall(MethodCall call, Result result) {
        ...
        switch (call.method) {
            ...
            case "writeCharacteristic": {
                ...
                System.out.println("writeCharacteristic - 数据" + Arrays.toString(request.getValue().toByteArray()));
                ...
                try {
                    ...
                    System.out.println("  characteristic.getProperties() = "+  characteristic.getProperties());

                    if ((characteristic.getProperties() & BluetoothGattCharacteristic.PROPERTY_WRITE) == 0
                            && (characteristic.getProperties()
                            & BluetoothGattCharacteristic.PROPERTY_WRITE_NO_RESPONSE) == 0) {
                        System.out.println("第一个条件就不满足");
                    }



                    Field mClientIfField = gattServer.getClass().getDeclaredField("mClientIf");
                    mClientIfField.setAccessible(true);
                    System.out.println("mClientIf 值 ?" + mClientIfField.get(gattServer));


                    System.out.println("characteristic.getValue() == null ?" + (characteristic.getValue() == null?
                    true:false));

                    BluetoothGattService service = characteristic.getService();
                    System.out.println("服务空 ?" + service == null ? true : false);
                    Field mDeviceField = service.getClass().getDeclaredField("mDevice");
                    mDeviceField.setAccessible(true);
                    BluetoothDevice mDevice = (BluetoothDevice) mDeviceField.get(service);
                    System.out.println("设备空 ?" + mDevice == null ? true : false);

                    Field mDeviceBusyField = gattServer.getClass().getDeclaredField("mDeviceBusy");
                    mDeviceBusyField.setAccessible(true);
                    boolean mDeviceBusy = (boolean) mDeviceBusyField.get(gattServer);
                    System.out.println("设备忙 ？" + mDeviceBusy);
                } catch (Exception e) {
